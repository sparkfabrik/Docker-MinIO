...
| `MINIO_TEMP_HOST`          | The address the temporary server used during the initialization binds to. It stays on the loopback so the bucket is not reachable while it is being seeded.              | `127.0.0.1`                         |
| `MINIO_TEMP_PORT`          | The port used by the temporary server during the initialization. It must differ from `MINIO_PORT`, otherwise the health check would report the container healthy too early; the entrypoint refuses to start on a collision. | `19000`                             |
| `MINIO_TEMP_CONSOLE_PORT`  | The console port used by the temporary server during the initialization. It must differ from `MINIO_PORT` as well; the entrypoint refuses to start on a collision.       | `19001`                             |
